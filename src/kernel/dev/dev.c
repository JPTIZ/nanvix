...
PUBLIC void bdev_readblk(buffer_t b) {
    bdev_readblk_syncable(b, TRUE);
}

/*
 * Reads a block from a block device.
 */
PUBLIC void bdev_readblk_syncable(buffer_t buf, unsigned sync)
{
    int err;   /* Error ?        */
    dev_t dev; /* Device number. */
    ...
    /* Read block. */
    err = bdevsw[MAJOR(dev)]->readblk(MINOR(dev), buf, sync);
    if (err)
